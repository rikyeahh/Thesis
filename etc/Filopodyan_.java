...
public class Filopodyan_ implements Command{
	...
	public Filopodyan_(int i_of_image) {
		super();
		this.I_of_image = i_of_image;
	}
	...
	*/	
	...
	try{
		...
		setImageVisible(true);
		ImagePlus image = proj;
		FileSaver fileSaver = new FileSaver(image);
		String outputPath = "C:\\Users\\ricca\\Desktop\\Thesis\\dataTools\\Filopodyan320\\"+String.valueOf(I_of_image)+".tif"; // Specify the output file path
	    if (fileSaver.saveAsTiff(outputPath)) {
	...
	try{
		...
		unit = imp.getCalibration().getUnit();
		if(pixelW==1){
			String str = "0.065";//(String)JOptionPane.showInputDialog(null,"Pixel size ("+unit+")",title,JOptionPane.PLAIN_MESSAGE,null,null, ""+defaultPixelW);
			if(str!=null){
				try{
	...
	public static void main(String[] arg) throws InterruptedException{
		...
		int l = 246;
		for(int i_of_image = l; i_of_image < l+1; i_of_image++) {
			ImageJ.main(arg);
			ImagePlus img = new ImagePlus("C:\\Users\\ricca\\Desktop\\Thesis\\IMAGESTESTStdNames320\\"+i_of_image+".tif");
			System.out.println(img.getNDimensions() + " "+ img.getNChannels());
			final ImagePlus image = img;//HyperStackConverter.toHyperStack(img, img.getNChannels(), 1, img.getNFrames());
			image.setDisplayMode(IJ.GRAYSCALE);
			image.show();
			new Filopodyan_(i_of_image).run();
			Thread.sleep(5000);
		}
	}
}
